...
public class PendapatanHarianFragment extends Fragment {

    private RecyclerView recyclerView;
    private PendapatanAdapter adapter;
    private BillingDatabaseHelper dbHelper;

    public PendapatanHarianFragment() {}

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_pendapatan_harian, container, false);
        recyclerView = view.findViewById(R.id.recyclerPendapatanHarian);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        dbHelper = new BillingDatabaseHelper(getContext());
        List<PendapatanItem> data = dbHelper.getPendapatanHarian();

        adapter = new PendapatanAdapter(data, true); // true untuk tampilkan hari
